...
8. Use `qstop` on SQL Server to rank Query Store consumers by total duration per `query_id` before measuring. A high rank is a lead to `measure`, `compare`, and `plan`, not proof the query is defective. Stdout has no SQL text.
9. Use `query` for a bounded read-only result.
10. Use `measure` for repeated timing, IO, and plan evidence for one query.
11. Use `compare` for baseline/candidate evidence and technical result equivalence.
12. Use `gain --json` to inspect recorded output savings.
